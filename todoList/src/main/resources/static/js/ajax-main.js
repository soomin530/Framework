...
/* fetch() API
  ...
  -> 어떤 결과가 올지는 모르지만 반드시 결과를 보내주겠다 라는 약속! 
  ...
  - 콜백 함수(callback function)는 특정 작업이나 이벤트가 완료된 후 호출되는 함수
    ...
    비동기 작업이 완료될 때까지 기다렸다가, 해당 작업의 결과를 이용하여
...
function getTotalCount() { // 함수 정의
  ...
  fetch("/ajax/totalCount") // 서버로 비동기 요청 보냄 (기본 - GET 요청)
  // 첫 번째 then (응답을 처리하는 역할)
  .then(response => { // fetch 가 서버에서 응답을 받으면
...
function getCompleteCount() { // 함수의 정의
  ...
  .then(response => response.text()) // 축약형 화살표 함수
...
const selectTodoList = () => {
  ...
  .then(todoList => {
    ...
    // 단순 텍스트이거나 JS Object 일 수도 있음

    // 만약 text() 사용했다면 문자열
    // -> JSON.parse() 이용하여 JS Object 타입으로 변환 가능 (JSON.parse(todoList);)
    ...
    console.log(todoList); // JS 객체 배열 형태로 출력(List로 넘어오고 있음)
    ...
    for(let todo of todoList) { // 향상된 for문
      ...
      for(let key of arr) { // 중첩 for문
        ...
        if(key === 'todoTitle') {
          ...
          // --> <a href="/ajax/detail?todoNo="+todo.todoNo></a>
          td.append(a);
          tr.append(td);
...
deleteBtn.addEventListener("click", () => {
  ...
  fetch("/ajax/delete", {
    ...
    // -> 원래는 이렇게 명시하는 게 옳음(엄격한 환경에서는 이렇게 작성해야 함)
  })
  .then(resp => resp.text())
  .then(result => {
...
changeComplete.addEventListener("click", () => {
  ...
  .then(result => {
    ...
    if(result > 0) { // 성공
      ...
      // getCompleteCount();
      // 서버 부하를 줄이기 위해 경우에 따라 완료된 Todo 개수 +-1
...
// 수정 레이어에서 취소버튼(#updateCancle)이 클릭되었을 때
...
getTotalCount(); // 함수 호출(호출을 꼭 해야 한다!)
