...
let previousImage = profileImg.src; // 이전 이미지 URL 기록해둘 용도로 사용할 변수 (초기 상태의 이미지 URL 저장)
let previousFile = null; // 이전의 선택된 파일 객체를 저장해둘 용도로 사용할 변수
...
imageInput.addEventListener("change", () => {
    ...
    if(file) { // 파일이 선택된 경우
        if(file.size <= MAX_SIZE) { // 파일 크기가 허용 범위 이내인 경우

            const newImageUrl = URL.createObjectURL(file); 
            // 임시 URL 생성 -> 미리보기 이미지 url 용도로 사용!
            profileImg.src = newImageUrl; // 미리보기 이미지 설정(img 태그의 src에 선택한 파일 임시 경로 대입)
            statusCheck = 1; // 새 이미지 선택 상태 기록
            previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장(다음에 바뀔 일에 대비) --> img src
            previousFile = file; // 현재 선택된 파일 객체를 이전 파일로 저장(다음에 바뀔 일에 대비) --> input files (최종적 제출은 이거!)
            

        } else { // 파일 크기가 허용 범위를 초과한 경우
            alert("5MB 이하의 이미지를 선택해주세요!");
            imageInput.value = ""; // 1. 파일 선택 초기화 (alert 창은 띄웠지만 이미 선택된 큰 사이즈 파일을 비우는 건 따로 해야 함)
            // == imageInpt.files = null;
            profileImg.src = previousImage; // 2. 이전 미리보기 이미지로 복원
            // 3. 파일 입력 복구 : 이전 파일이 존재하면 다시 할당
            if(previousFile) { // previousFile이 존재한다면
...
deleteImage.addEventListener("click", () => {
    ...
    if(profileImg.src !== defaultImageUrl) { // 현재 프로필 이미지가 기본 이미지가 아닐 때
