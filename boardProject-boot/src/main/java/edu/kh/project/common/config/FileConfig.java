...
public class FileConfig implements WebMvcConfigurer {
	...
	private String boardResourceLocation; // file:///C:/uploadFile/board
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		.addResourceLocations("file:///C:/uploadFiles/test/"); // 요청을 연결해서 처리해 줄 서버 폴더 경로
		...
		.addResourceLocations(boardResourceLocation); // file:///C:/uploadFile/board
	...
	public MultipartConfigElement configElement() {
		...
		// 업로드 파일의 최대 크기, 메모리 에서의 임시 저장경로 등 설정 가능
	...
	// -> Bean으로 등록하면서 위에서 만든 MultipartConfigElement를 자동으로 이용함
	// MultipartConfigElement 기능과 더해서 사용!
	
	@Bean
	public MultipartResolver multipartResolver() {
		// multipartResolver : MultipartFile을 처리해주는 해결사
		// multipartResolver는 클라이언트로 부터 받은 multipart 요청을 처리하고,
		// 이중에서 업로드 된 파일을 추출하여 MultipartFile 객채로 제공하는 역할
