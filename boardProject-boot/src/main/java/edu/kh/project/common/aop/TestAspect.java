...
@Component // Bean 등록
// @Aspect // 공통 관심사가 작성된 클래스임을 명시 (AOP 동작용 클래스다~ 정의)
@Slf4j // log를 찍을 수 있는 객체(Logger) 생성 코드를 추가 (Lombok에서 제공) 
public class TestAspect {
	...
	// .* : 모든 메서드
	// (..) : 0개 이상의 파라미터를 나타냄
	// -> edu.kh.project 패키지 아래에 있는 컨트롤러라는 이름을 가진 모든 클래스의 모든 메서드의 파라미터
