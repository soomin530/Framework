...
public class ImageDeleteScheduling {
	...
	public void scheduling() {
		...
		File[] boardArr = boardFolder.listFiles(); // 폴더 안 파일 목록을 List 형태로 반환해 저장
		File[] memberArr = memberFolder.listFiles();
		
		// 두 배열을 하나로 합치기
		// imageArr 라는 빈 배열을 boardArr과 memberArr 길이만큼의 크기로 만든다
		File[] imageArr = new File[boardArr.length + memberArr.length]; 
		
		// 배열 내용 복사(깊은 복사)
		// System.arraycopy(복사할배열, 몇번인덱스부터복사할지, 새로운배열,
		// 					새로운배열의몇번부터넣을지, 복사를어디까지할건지)  
