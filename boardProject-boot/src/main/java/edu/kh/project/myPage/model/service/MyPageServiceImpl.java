...
public class MyPageServiceImpl implements MyPageService {
	...
	// BCrypt encoder 라는 암호화 객체 사용하려면 의존성 주입 받아야 함!!
	...
	public int changePw(Map<String, Object> paramMap, int memberNo) {
		...
		// 우리가 입력한 평문 암호는 암호화 되어있는 DB에 있는 암호와 비교할 수 없기 때문에
		// 현재 로그인 한 회원 번호 전달해서
		// MemberNo가 가지고 있는(DB에 입력되어 있는) 암호화된 비번 조회해서 가져와야 한다.
	...
	public int fileUpload2(int memberNo, MultipartFile uploadFile) throws Exception {
		...
		// 1. 서버에 저장할 파일 경로 만들기
				...
				//log.debug("fileRename : " + fileRename); 20241112101904_00001.jpg
...
//				uf2.fileRename(fileRename);
				...
				uploadFile.transferTo(new File(folderPath + fileRename));
							// C:/uploadFiles/test/20241112101904_00001.jpg
	...
	public int fileUpload3(List<MultipartFile> aaaList,
		...
		// 업로드 된 파일이 없을 경우를 제외하고 업로드
		for(MultipartFile file : aaaList) {
			
			if(file.isEmpty()) { // 파일이 없으면 다음 파일로
				continue; // 아래 코드 수행하지 않고 넘어감
			}
			
			// fileUpload2() 메서드 호출(재활용)
			// -> 파일 하나 업로드하면서 DB INSERT 까지 해줌
		...
		for(MultipartFile file : bbbList) {
			...
			if(file.isEmpty()) { // 파일이 없으면 다음 파일로 넘어가겠다
				continue; 
			}
	...
	public int profile(MultipartFile profileImg, Member loginMember) throws Exception {
		...
		Member mem = Member.builder()
					...
					.profileImg(updatePath) // 업로드 이미지가 없다면 updatePath == null
					.build();
