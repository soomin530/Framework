...
public class EditBoardServiceImpl implements EditBoardService{
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) throws Exception {
		...
		// 삽입 성공 시
		// 삽입된 게시글의 번호를 변수로 저장
		// -> mapper.xml에서 <selectKey> 태그를 이용해서 생성된 (keyProperty="boardNo")
		// 	  boardNo가 inputBoard에 저장된 상태 !!!
		// 	  (얕은 복사 개념 이해 필수)
		
		// 깊은 복사 : 안에 값만 복사. 주소값 같지 않음
		// 얕은 복사 : 주소값 복사 주소값 같음
		int boardNo = inputBoard.getBoardNo();
		
		
		// 2. 업로드 된 이미지가 실제로 존재할 경우
		// 	  업로드 된 이미지만 별도로 저장하여
		//	  BOARD_IMG 테이블에 삽입하는 코드 작성
		// 	  (매개변수로 받고 있는 List는 5개 존재하긴 하지만
		//	   비워져 있을 수도 있고 채워져있을 수도 있어서 걸러야 하기 때문!)
		
		// 실제 업로드 된 이미지의 정보를 모아둘 List 생성
		List<BoardImg> uploadList = new ArrayList<>();
		
		// images 리스트에서 하나씩 꺼내 파일이 있는 지 검사
		for(int i = 0; i < images.size(); i++) {
			
			// 실제 선택된 파일이 존재하는 경우
			if(!images.get(i).isEmpty()) { // 비어있지 않은 경우
		...
		if(uploadList.isEmpty()) {
			return boardNo; // 컨트롤러로 현재 제목/상세내용 (BOARD) 삽입된 게시글 번호만 넘김
		}
		...
		if(result == uploadList.size()) { // 성공
			...
			// rollback 하는 방법
			// == RuntimeException 강제 발생 (@Transactional이 잡아줌)
	...
	public int boardUpdate(Board inputBoard, List<MultipartFile> images, String deleteOrderList) throws Exception {
		...
		if(deleteOrderList != null && !deleteOrderList.equals("")) { // null이고 빈 문자열이 아닐 때(앞에 ! 빼먹지 말기)
