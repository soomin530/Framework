...
public class EditBoardController {
	...
	public String boardInsert(@PathVariable("boardCode")int boardCode) { 
		...
		return "board/boardWrite"; // templates/board/boardWrite.html로 forward (boardCode 실려있음)
		
	}
	
	/** 게시글 작성 후 등록
	 * @param : boardCode : 어떤 게시판에 작성할 글인지 구분 (1/2/3..)
	 * @param : inputBoard : 입력된 값(제목, 상세내용) 세팅되어있음 (커멘드 객체) 
	 * @param : loginMember : 로그인 한 회원 번호를 얻어오는 용도(세션에 등록되어있음)
	 * @param  images : 제출된 file 타입 input 태그가 전달할 데이터들(이미지 파일..)
	...
	public String boardInsert(@PathVariable("boardCode")int boardCode,
		 ...
		 *   이렇게 다 제출은 되어있지만 2번 인덱스만 업로드할 시 2번 인덱스 쪽만 데이터 있음
		 *   
		 *   -> 파일 선택이 안 된 input 태그 값을 서버에 저장하려고 하면 오류 발생함!
		 *   
		 * [해결방법]
		 * - 무작정 서버에 저장 X
		 * -> List의 각 인덱스에 들어있는 MultipartFile에
		 *    실제로 제출된 파일이 있는 지 확인하는 로직을 추가 구성
		 *     
		 *  + List 요소의 index 번호 == IMG_ORDER와 같음
		 *  
		 * */
		
		// 1. boardCode, 로그인 한 회원번호를 inputBoard(커맨드 객체로 얻어온)에 세팅 
		inputBoard.setBoardCode(boardCode);
		inputBoard.setMemberNo(loginMember.getMemberNo());
		// -> inputBoard 총 네 가지 세팅됨(boardTitle, boardContent, boardCode, memberNo) 
		...
		if(boardNo > 0) {
			...
			path = "/board/" + boardCode + "/" + boardNo;  // /board/1/2002  -> 상세조회 경로
			message = "게시글이 작성되었습니다";
			
		} else {
			
			path = "insert"; // 상대경로로 작성, /editBoard/1/insert
			message = "게시글 작성 실패";
	 ...
	 * @param boardNo : 현재 게시글 번호
	 * @param loginMember : 로그인 한 회원이 작성한 글이 맞는지 검사하는 용도
	 * @param model : 포워드 시 request scope 로 값 전달하는 용도
	 * @param ra : 리다이렉트 시 request scope 로 값 전달하는 용도
	 * @return
	 */
	// /editBoard/1/2001/update?cp=1
	@GetMapping("{boardCode:[0-9]+}/{boardNo:[0-9]+}/update") // 숫자만 들어올 수 있다 
	 ...
	 * @param loginMember      : 로그인 한 회원 번호 이용 (로그인 == 작성자)
