...
public class EmailServiceImpl implements EmailService {
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			helper.setText(loadHtml(authKey, htmlName), true); // HTML 내용을 설정 
													  // ㄴ html 코드 해석해서 보내겠다 (기본은 false, false일 시 문자열 상태로 반환)
			// 인증번호입니다 : ag2cd 출력 원할 시
			// helper.setText("인증번호입니다 : " + authKey); 사용
			// 메일에 이미지 첨부 (로고)
			...
			return authKey; // 모든 작업 성공 시 인증키 반환 (Controller 쪽으로)
	...
	private String loadHtml(String authKey, String htmlName) {
		...
		// Context : Thymeleaf에서 제공하는 HTML템플릿에 데이터를 전달하기 위해 사용하는 클래스
		
		Context context = new Context();
		context.setVariable("authKey", authKey);  
		
		return templateEngine.process("email/" + htmlName, context); // 28번 라인 
	}
	...
	private boolean storeAuthKey(Map<String, String> map) {
		...
		return result > 0; // 성공 여부 반환 (true or false) 
